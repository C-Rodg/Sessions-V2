...
export class SessionsService {
	...
	// Upload single scan
	upload(scan) {
		const url = `${this.infoService.event.Event
			.SessionUrl}/UploadSessionScan/${EventGuid.guid}`;
		let {
			SessionScanGuid,
			SessionGuid,
			DeviceId,
		...
		if (!deviceName || deviceName === "-NO DEVICE NAME-") {
		...
		headers.append(
			"Authorization",
			`ValidarSession token="${this.infoService.getCurrentToken()}"`
		);
		return this.http
			.post(url, d, { headers })
			.map(res => res.json())
	...
	postAndUpload(scan) {
		const sessionUrl = `${this.infoService.event.Event
			.SessionUrl}/UploadSessionScan/${EventGuid.guid}`;
		let {
			SessionScanGuid,
			SessionGuid,
		...
		const pwsUrl =
			"https://registration.validar.com/WebServices/V1/Partner/PartnerService.asmx";
		let pwsHeaders = new Headers();
		pwsHeaders.append("Content-Type", "text/xml; charset=utf-8");
		pwsHeaders.append(
			"SOAPAction",
			"https://portal.validar.com/PutRegistrationData"
		);
		...
		const pwsBody =
			...
			"<Username>SessionCheckin</Username>" +
			"<Password>8YbQwoD%Z!1k</Password>" +
		...
		const sessionReq = this.http
			.post(sessionUrl, sessionData, { headers: sessionHeaders })
			.map(res => res.json())
			.catch(err => {
		...
		const pwsReq = this.http
			.post(pwsUrl, pwsBody, { headers: pwsHeaders })
			.map(res => res.text())
			.catch(err => {
				return Observable.of({ error: true });
			});

		return Observable.forkJoin([sessionReq, pwsReq])
			.flatMap(requests => {
				if (
					(requests[0].hasOwnProperty("error") && requests[0].error) ||
					(requests[1].hasOwnProperty("error") && requests[1].error)
				) {
	...
	uploadAllPending() {
		...
		return this.all()
			.flatMap((sessions: any) => {
				const checkIns = sessions.filter(sesh => {
					return (
						sesh.Category.toUpperCase().replace(/\W+/g, "") === "CHECKIN" &&
						sesh.Description
					);
				});
				...
				if (!checkIns) {
					...
					return this.getAllScans("uploaded=no&error=no").flatMap(data => {
						if (data && data.length > 0) {
							const uploadRequests = data.map(scan => {
								if (
									scan.SessionGuid.toUpperCase() ===
									checkIns["SessionGuid"].toUpperCase()
								) {
									scan["HannahGuid"] = checkIns["Description"];
									return this.postAndUpload(scan)
										.map(d => d)
										.catch(err => {
											return Observable.of({ error: true });
										});
	...
	fetchSessions() {
		...
		return this.http
			.get(
				`${this.infoService.event.Event
					.SessionUrl}/ListAttendanceTrackingScheduleItemSessions/${EventGuid.guid}`,
				{ headers }
			)
			.map(res => res.json())
			.map(res => {
	...
	fetchAccess(scheduleItemGuid) {
		...
		return this.http
			.get(
				`${this.infoService.event.Event
					.AccessControlUrl}/${EventGuid.guid}/SessionAccessList/${scheduleItemGuid}`,
				{ headers }
			)
			.map(res => res.json())
			.map(res => {
	...
	saveAccessList(sessionGuid, registrantRecords) {
		return this.http
			.put(
				`http://localhost/events/${EventGuid.guid}/sessions/${sessionGuid}/accesslist`,
				JSON.stringify(registrantRecords)
			)
			.map(res => res.json())
			.map(res => {
				if (res.Fault) {
	...
	sessionCountCentral(scheduleItemGuid) {
		...
		return this.http
			.get(
				`${this.infoService.event.Event
					.SessionUrl}/CountAttendeesAtSession/${EventGuid.guid}/${scheduleItemGuid}`,
				{ headers }
			)
			.map(res => res.json())
			.map(res => {
	...
	all() {
		return this.http
			.get(`http://localhost/events/${EventGuid.guid}/sessions`)
			.map(res => res.json())
			.map(res => {
				if (res.Fault) {
					return Observable.throw(res.Fault);
				} else {
					this.allSessions = this.sortByStartDate(
						this.convertListToDisplaySession(res.Sessions)
					);
	...
	saveScan(scan) {
		return this.http
			.put(
				`http://localhost/events/${EventGuid.guid}/sessions/scans`,
				JSON.stringify(scan)
			)
			.map(res => res.json())
			.map(res => {
	...
	markUploaded(scanGuid) {
		return this.http
			.put(
				`http://localhost/events/${EventGuid.guid}/sessions/scans/${scanGuid}/uploaded`,
				{}
			)
			.map(res => res.json())
			.map(res => {
	...
	markError(scanGuid) {
		return this.http
			.put(
				`http://localhost/events/${EventGuid.guid}/sessions/scans/${scanGuid}/error`,
				{}
			)
			.map(res => res.json())
			.map(res => {
				if (!res) {
					return Observable.throw(
						"Invalid response object returned by the ajax call"
					);
				}
				return res;
			});
	...
	searchSessions(options) {
		return this.http
			.get(`http://localhost/events/${EventGuid.guid}/sessions?${options}`)
			.map(res => res.json())
			.map(res => {
				if (res.Fault) {
					return Observable.throw(res.Fault);
	...
	countSessions(options) {
		return this.http
			.get(
				`http://localhost/events/${EventGuid.guid}/sessions/count?${options}`
			)
			.map(res => res.json())
			.map(res => {
				if (res.Fault) {
	...
	get(sessionGuid) {
		if (this.allSessions.length > 0) {
			...
			return this.http
				.get(
					`http://localhost/events/${EventGuid.guid}/sessions/${sessionGuid}`
				)
				.map(res => res.json())
				.map(res => {
					if (res.Fault) {
	...
	getAccess(sessionGuid, badgeId) {
		return this.http
			.get(
				`http://localhost/events/${EventGuid.guid}/sessions/${sessionGuid}/accessList/${badgeId}`
			)
			.map(res => res.json());
	}
	...
	getAccessList(sessionGuid, options) {
		return this.http
			.get(
				`http://localhost/events/${EventGuid.guid}/sessions/${sessionGuid}/accessList?${options}`
			)
			.map(res => res.json());
	}
	...
	getScans(sessionGuid, options) {
		return this.http
			.get(
				`http://localhost/events/${EventGuid.guid}/sessions/${sessionGuid}/scans?${options}`
			)
			.map(res => res.json())
			.map(res => {
				if (res.Fault) {
					...
					res.hasOwnProperty("SessionScans") &&
					res.SessionScans !== null
				) {
	...
	getAllScans(options) {
		return this.http
			.get(
				`http://localhost/events/${EventGuid.guid}/sessions/scans?${options}`
			)
			.map(res => res.json())
			.map(res => {
				if (res.Fault) {
	...
	getTotalCount(options) {
		return this.http
			.get(
				`http://localhost/events/${EventGuid.guid}/sessions/scans/count?${options}`
			)
			.map(res => res.json())
			.map(res => {
				if (res.Fault) {
	...
	getCount(sessionGuid, options?) {
		return this.http
			.get(
				`http://localhost/events/${EventGuid.guid}/sessions/${sessionGuid}/scans/count?${options}`
			)
			.map(res => res.json())
			.map(res => {
				if (res.Fault) {
	...
	getAccessCount(sessionGuid) {
		return this.http
			.get(
				`http://localhost/events/${EventGuid.guid}/sessions/${sessionGuid}/accessList/count`
			)
			.map(res => res.json())
			.map(res => {
				if (res.Fault) {
	...
	getUniqueLocations() {
		return this.http
			.get(`http://localhost/events/${EventGuid.guid}/sessions/locations`)
			.map(res => res.json())
			.map(res => {
				if (res.Fault) {
					return Observable.throw(res.Fault);
	...
	getUniqueStartDates() {
		return this.http
			.get(`http://localhost/events/${EventGuid.guid}/sessions/startdates`)
			.map(res => res.json())
			.map(res => {
				if (res.Fault) {
					return Observable.throw(res.Fault);
	...
	saveSessionList(sessions) {
		...
		return this.http
			.put(
				`http://localhost/events/${EventGuid.guid}/sessions`,
	...
	refreshAccessListsAndSave() {
		...
		const accessListRequests = accessControlSessions.map(session => {
			return this.fetchAccess(session.SessionGuid)
				.map(d => d)
				.catch(err => {
					return Observable.of(null);
				});
	...
	refreshSessionsThenAccessLists() {
		return this.refreshSessionsAndSave()
			.flatMap(() => this.refreshAccessListsAndSave())
			.flatMap(() => {
				window.localStorage.setItem(
					`${EventGuid.guid}_sync`,
	...
	refreshRoomsAndDates() {
		return this.getUniqueLocations()
			...
			.flatMap(() => this.getUniqueStartDates())
			.map(d => {
				if (d && d.StartDates) {
	...
	backgroundUploadAction() {
		...
		this.uploadAllPending().subscribe(
			data => {
				// Do nothing...
			},
			err => {
	...
	private convertListToDisplaySession(arr) {
		return arr.map(this.convertToDisplaySession).filter(this.removeBLEsessions);
	}

	// Remove BLE Sessions
	private removeBLEsessions(session) {
		if (session !== null) {
			return session.Category.toUpperCase().replace(/ /g, "") !== "BLE"
				? true
				: false;
		}
		return false;
	...
	private convertToDisplaySession(session) {
		if (session !== null) {
			if (session.hasOwnProperty("Topic") && session.Topic.indexOf("|") > 0) {
				session.Topic = session.Topic.substr(
					session.Topic.indexOf("|") + 1,
					session.Topic.length
				);
			}
			if (
				session.hasOwnProperty("StartDateTime") &&
				session.StartDateTime !== null
			) {
				session.StartDateTime = session.StartDateTime.replace(/Z/, "");
			}
			if (
				session.hasOwnProperty("EndDateTime") &&
				session.EndDateTime !== null
			) {
				session.EndDateTime = session.EndDateTime.replace(/Z/, "");
			}
			const AccessControl =
				session.Category.toUpperCase().replace(/ /g, "") === "ACCESSCONTROL"
					? true
					: false;
			...
			const CheckInMode =
				session.Category.toUpperCase().replace(/\W+/g, "") === "CHECKIN" &&
				session.Description
					? true
					: false;
