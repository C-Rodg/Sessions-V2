...
export class SettingsService {
	...
	constructor(private infoService: InformationService) {
		...
		if (
			(!this.deviceName || this.deviceName === "-NO DEVICE NAME-") &&
			clientName
		) {
	...
	storeCurrentSettings() {
		...
		if (!settings.deviceName || settings.deviceName === "-NO DEVICE NAME-") {
			settings.deviceName = this.infoService.getClientName();
		}
