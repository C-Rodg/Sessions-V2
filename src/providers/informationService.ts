...
export class InformationService {
	...
	getAuthToken() {
		return this.http
			.get(`http://localhost/events/${EventGuid.guid}/sessiontoken`)
			.map(res => res.json())
			.map(res => {
	...
	saveToken(loginArgs) {
		...
		return this.http
			.put(
				`http://localhost/events/${EventGuid.guid}/sessiontoken`,
				this.currentToken
			)
			.map(res => res.json());
	}
	...
	computeHash(loginArgs) {
		...
		return this.http
			.post(`http://localhost/digestauthentication/computehash`, req)
			.map(res => res.json())
			.map(res => {
	...
	getEventInformation() {
		return this.http
			.get(`http://localhost/events/${EventGuid.guid}`)
			.map(res => res.json())
			.map(res => {
				this.event = res;
				return res;
	...
	getClientInfo() {
		return this.http
			...
			.catch(err => {
				console.log("error in getClientInfo");
				console.log(err);
	...
	getLineaStatus(): boolean {
		return !this.client.Scanner || this.client.Scanner === "None"
			? false
			: true;
	}
